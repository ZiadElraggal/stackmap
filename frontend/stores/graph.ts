...
export const useGraphStore = defineStore('graph', {
  ...
  getters: {
    ...
    visibleNodes(state): StackMapNode[] {
      ...
      return this.graphNodes.filter((n: StackMapNode) => {
        ...
        // Diff mode: hide nodes that have no opacity at the current slider position
        if (state.diffMode) {
          const diffStatus = n.position_hint?.diff_status as string | undefined
          if (state.showOnlyChanges && diffStatus === 'unchanged') return false
          // At slider=0: hide added nodes; at slider=1: hide removed nodes
  ...
  actions: {
    async loadFromJSON(path: string) {
      ...
      // Auto-enable diff mode when the IR was generated by `stackmap diff --format html`
